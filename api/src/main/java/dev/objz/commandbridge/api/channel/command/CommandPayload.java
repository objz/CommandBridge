 ...
 *
 * <p>The three components define what to run and how. {@code command} is the command string
 * without a leading slash. {@code runAs} defines the execution context; see {@link RunAs} for
 * available modes. {@code player} is the optional player UUID required when {@code runAs} is
 * {@link RunAs#PLAYER} or {@link RunAs#OPERATOR}; pass {@code null} for {@link RunAs#CONSOLE}.
 *
 * <p>Instances are sent through a
 ...
 * }</pre>
 *
 * @param command the command string to execute on the target server; must not include a leading
 *                slash. Must not be {@code null}.
 * @param runAs   the execution context defining which identity and permissions are used; must not
 *                be {@code null}. See {@link RunAs} for available modes.
 * @param player  the UUID of the player to execute the command as, or {@code null} when
 *                {@code runAs} is {@link RunAs#CONSOLE}. Required when {@code runAs} is
...
public record CommandPayload(String command, RunAs runAs, UUID player) implements ChannelPayload {
     ...
     *
     * <p>Use this constructor when {@code runAs} is {@link RunAs#CONSOLE}, as no player UUID
     * is required.
     *
     * @param command the command string to execute; must not include a leading slash
