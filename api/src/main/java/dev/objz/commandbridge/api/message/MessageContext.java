 ...
 *
 * <p>
 * {@code channel} is the payload class token identifying the channel the
 * message arrived on.
 * {@code from} identifies the server that sent the message.
 * {@code timestamp} is the Unix epoch millisecond value when the message was
 * sent.
 *
 * @param <T>       the payload type of the channel this message was received
 *                  on;
 *                  bounded by
 *                  {@link dev.objz.commandbridge.api.channel.ChannelPayload}
 * @param channel   the {@link Class} token of the payload type, identifying
 *                  which channel
 *                  this message was received on
 * @param from      the {@link Platform.ServerTarget} identifying the server
