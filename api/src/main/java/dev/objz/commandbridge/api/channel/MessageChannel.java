...
public interface MessageChannel<P extends ChannelPayload> {
    ...
    /**
     * Returns a {@link Sender} that broadcasts to all servers currently connected
     * to the bridge
     * network.
     *
     * @return a {@code Sender} targeting every currently connected server. The set
     *         of servers is
     ...
     *
     * @param listener the callback to invoke for each incoming message; must not be
     *                 {@code null}
     * @return a {@link dev.objz.commandbridge.api.message.Subscription} that can be
     *         cancelled via
     *         {@link dev.objz.commandbridge.api.message.Subscription#cancel()} to
     *         stop receiving
    ...
    /**
     * A target-bound sender created by
     * {@link MessageChannel#to(java.util.Collection)} or
     * {@link MessageChannel#toAll()}, used to dispatch payloads to the targeted
     * servers.
     *
     * <p>
     * Obtain a {@code Sender} by calling one of the {@link MessageChannel} target
     * methods.
     * Send a payload with {@link #send(ChannelPayload)} for fire-and-forget
     * delivery, or use
     * {@link #request(ChannelPayload)} for request-response patterns. Optional
     * delivery conditions
    ...
    interface Sender<P extends ChannelPayload> {
         ...
         * @return a {@link java.util.concurrent.CompletableFuture}{@code <Void>} that
         *         completes
         *         when the payload has been dispatched (not when it has been received
         *         or processed
        ...
        /**
         * Sends a payload and returns a future that completes with the remote server's
         * response,
         * using the default timeout.
         *
         * <p>
         * This method is only supported for single-target senders. Invoking it on a
         ...
         * @return a {@link java.util.concurrent.CompletableFuture} that completes with
         *         the
         *         response payload, or completes exceptionally if the default timeout
         *         elapses
         *         before a response is received
         * @throws UnsupportedOperationException if this sender targets more than one
         *                                       server
         ...
         *
         * @param payload the request payload; must not be {@code null}
         * @param timeout the maximum duration to wait for a response; must not be
         *                {@code null}
         * @return a {@link java.util.concurrent.CompletableFuture} that completes with
         *         the
        ...
        /**
         * Constrains this sender to only execute if the specified player is present on
         * the target
         * server at the time of delivery.
         *
         * <p>
         * If the player is not present, the command is not delivered to that server.
         * Use
         * {@link #whenOnline(UUID)} instead if queued delivery when the player connects
         * is
         * desired.
         *
         * @param player the UUID of the player whose presence is required; must not be
         *               {@code null}
         * @return this sender for method chaining
         * @throws UnsupportedOperationException if this sender implementation does not
         *                                       support
         *                                       player-conditioned delivery (the
         *                                       default
        ...
        /**
         * Constrains this sender to only execute if the triggering player (if any) is
         * present on
         * the target server at the time of delivery.
         *
         * <p>
         * This is the no-argument variant of {@link #requirePlayer(UUID)}; it uses the
         * player
        ...
        /**
         * Queues the payload for delivery to the target server when the specified
         * player is online.
         *
         * <p>
         * Unlike {@link #requirePlayer(UUID)}, which skips delivery if the player is
         * absent,
         * this method queues the payload and delivers it once the player connects to
         * the target
         * server.
         *
        ...
        /**
         * Queues the payload for delivery to the target server when the triggering
         * player (if any)
         * is online.
         *
         * <p>
         * This is the no-argument variant of {@link #whenOnline(UUID)}; it uses the
         * player
         * associated with the execution context rather than an explicit UUID.
         *
         * @return this sender for method chaining
