...
public final class ExecuteCommandHandler extends InboundHandler {
    ...
    public void accept(Endpoint endpoint, Envelope env) {
        if (env.payload() == null) {
            ...
            sendFailure(endpoint, env, null, null, "Null payload received");
            return;
        }
        ...
        try {
            ...
            sendFailure(endpoint, env, null, null, "Failed to parse command:  " + e.getMessage());
            return;
        }

        if (exec == null || exec.command() == null || exec.command().isBlank()) {
            Log.warn("Received empty EXECUTE_COMMAND from '{}'", env.from());
            sendFailure(endpoint, env, "", null, "Empty command received");
            return;
        }
        ...
        executor.execute(exec, finalPermissions)
                .thenAccept(result -> {
                    if (result.isSuccess()) {
                        ...
                        sendSuccess(endpoint, env, exec.command(), exec.uuid());
                    } else {
                        Log.warn("Command '{}' execution failed: {}", exec.command(),
                                result.message());
                        sendFailure(endpoint, env, exec.command(), exec.uuid(), result.message());
                    }
                })
                .exceptionally(ex -> {
                    Log.error(ex, "Command '{}' execution threw exception", exec.command());
                    sendFailure(endpoint, env, exec.command(), exec.uuid(),
                            "Exception:  " + ex.getMessage());
                    return null;
                });
    }

    private void sendSuccess(Endpoint endpoint, Envelope env, String command, java.util.UUID playerUuid) {
        ExecuteCommandResult result = ExecuteCommandResult.success(command, playerUuid);
        reply(endpoint, env, MessageType.EXECUTE_COMMAND_RESULT, result)
                .dispatch()
                .exceptionally(ex -> {
    ...
    private void sendFailure(Endpoint endpoint, Envelope env, String command, java.util.UUID playerUuid,
            String message) {
